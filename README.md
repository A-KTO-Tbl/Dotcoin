...
![Screenshot_1](https://telegra.ph/file/06d376e9c05f92b5aad26.png)

# 🟣 Dotcoin AutoBot /// HPV /// V1.04 🟣

| **Возможности**                                                    | **Поддерживается**  |
|----------------------------------------------------------------|:---------------:|
| **Многофункциональный** — *выполняет все те же действия, что вы делали бы руками:*<br>**—** *Получение бонуса*;<br>**—** *Улучшение бустов ("Multitap" и "Daily Attempts")*;<br>**—** *Улучшение майнинга DTC* 🆕;<br>**—** *Получение кол-ва доступных игр и запуск их прохождения*;<br>**—** *Просмотр рекламы и отыгрыш полученных игр*;<br>**—** *Ожидание от 8 до 9 часов*;<br>**—** *Повторение действий через 8-9 часов*. |✅|
...
| **Proxy** — *скрипт поддерживает добавление неограниченного количества прокси-серверов. Даже если количество прокси меньше, чем количество аккаунтов, они будут равномерно распределены между всеми аккаунтами после проверки на валидность. Хотя использование прокси не является обязательным, настоятельно рекомендуется их использование для предотвращения блокировки в Dotcoin (не в Telegram)* |✅|
| **Точечная настройка** — *скрипт позволяет настроить желаемое количество монет, получаемых за одну игру. В конфигурации можно указать диапазон чисел, в пределах которого будут начисляться поинты. Помимо этого, вы можете настроить автоматическое обновление бустов до заданного уровня. По умолчанию бусты "Multitap" и "Daily Attempts" обновляются до уровня 11, что считается оптимальным решением, а желаемое количество монет за игру установлено от 17,500 до 20,000. Увеличивать максимальное значение выше 20,000 не рекомендуется. В лучшем случае монеты не будут засчитаны на баланс, в худшем — ваш аккаунт может быть заблокирован в Dotcoin (не в Telegram).* |✅|
...
- **Linux**
  ...
  - ```
    ...
    ```
  - ```
    git clone https://github.com/A-KTO-Tbl/Dotcoin
    ```
  - ```
    pip3 install -r Dotcoin\Core\Tools\requirements.txt
    ```
- **Windows**
  ...
  - Запустится CMD в текущей директории. Далее просто вводим следующие команды:
    - ```
      ...
      ```
    - ```
      pip install -r Dotcoin\Core\Tools\requirements.txt
      ```
- **Android**
